...
#define GF_SPIDEV_NAME     "goodix,side-fingerprint"
...
#endif
{
	...
	printk("[SIDE_FP] gf_power_on: set FP_3V3_EN +++\n");
	if (gf_power_on(gf_dev) < 0) {
		printk("[SIDE_FP] opps gf_power_on fail! \n");
		status = -4;
		goto error_hw;
	}
	printk("[SIDE_FP] gf_power_on: set FP_3V3_EN ---\n");
